...
TEST_CASE("C generator initializes scalar declarations", "[codegensi]")
{
    ...
    CHECK(output.contains("avium_codegen_text_copy(avium_empty_text, 5)"));
    CHECK(output.contains("avium_text_copy(&result, &value, line);"));
...
TEST_CASE("C generator emits reusable escaped text values", "[codegensi]")
{
    ...
    CHECK(output.contains("avium_codegen_text_copy("));
}
...
TEST_CASE("C generator copies stored text values and moves temporary results", "[codegensi]")
{
    ...
    CHECK(output.contains("avium_codegen_text_copy(first, 5)"));
    CHECK(output.contains(" = avium_codegen_input(6);"));
    CHECK_FALSE(output.contains("avium_codegen_text_copy(avium_codegen_input"));
    CHECK(output.contains("avium_text_move_assign(&first"));
    CHECK(output.contains("avium_text_move_assign(&second"));
}

TEST_CASE("C generator converts boolean values to text", "[codegensi]")
{
    const auto output = generate(
        "SUB Main\n"
        "DIM text AS TEXT\n"
    ...
    CHECK(output.contains("avium_codegen_str_bool(true, 3)"));
    CHECK(output.contains("avium_str_bool(&result, value, line);"));
}
...
TEST_CASE("C generator cleans local objects before returning text", "[codegensi]")
{
    ...
    const auto copy = output.find("avium_codegen_text_copy(local, 4)");
